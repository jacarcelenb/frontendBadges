...
@Component({
  ...
  styleUrls: ['./reproduced-narrative-file.component.scss']
})
export class ReproducedNarrativeFileComponent implements OnInit {
 ...
 getValueEvaluation(){
  ...
  generatePDFfile(){
   ...
   if (this.textfile.nativeElement.value.trim().length == 0) {
    ...
    autoTable(doc, {
      body: [
        [
          {
            content: 'Reproduced Narrative File',
            styles: {
              halign: 'left',
              fontSize: 9,
              fontStyle: 'bold',
    ...
    autoTable(doc, {
      body: [
        [
          ...
          {
            content: 'The Reproduced Narrative File describes in a comprensible way how to make the reproduction for the original experiment.',
          }
   ...
   // this.createEvaluationStandard()
    return doc.save("Reproduced_Narrative_File.pdf")
   }
  }
...
save(file_url, file_content) {
  ...
  const artifact = {
    name: 'Archivo narrativa reproducido',
    file_content: 'Archivo narrativa reproducido',
