...
@Component({
  ...
  styleUrls: ['./contact-file.component.scss']
})
export class ContactFileComponent implements OnInit {
  ...
  getUploadedArtifacts() {
    this._artifactService.get({ name: "Archivo contact", is_acm: true, experiment: this.id_experiment  }).subscribe((data: any) => {
...
changeDate(date: any): string {
  ...
  generatePDFfile(){
    ...
    autoTable(doc, {
      body: [
        [
          {
            content: 'This CONTACT file describes the information from the authors of the experiment.',
          }
  ...
  save(file_url, file_content, isGenerated) {
    ...
    const artifact = {
      name: 'Archivo contact',
      file_content: 'Archivo contact',
