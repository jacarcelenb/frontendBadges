...
@Component({
  selector: 'app-hardware-requirements',
  templateUrl: './hardware-requirements.component.html',
  styleUrls: ['./hardware-requirements.component.scss']
})
export class HardwareRequirementsComponent implements OnInit {
  standard_name = "hardware_technical_requirement";
  ...
  getUploadedArtifacts() {
    this._artifactService.get({ name: "Hardware technical requirement", is_acm: true, experiment: this.id_experiment  }).subscribe((data: any) => {
  ...
  generatePDFfile(artifact){
    ...
    autoTable(doc, {
      body: [
        [
          {
            content: 'Hardware technical requirement File',
            styles: {
              halign: 'left',
              fontSize: 9,
              fontStyle: 'bold',
    ...
    autoTable(doc, {
      body: [
        [
          ...
          {
            content: 'Hardware technical requirement file explain the requirements necessary to make use of the hardware used in the experiment are explained.',
          }
    ...
    let fileData = new File([blobPDF], "Hardware_technical_requirement.pdf", { type: blobPDF.type })
...
save(file_url, file_content,isGenerated) {
  ...
  const artifact = {
    name: 'Hardware technical requirement',
    file_content: 'Hardware technical requirement',
